...
import argparse, json, math, sys, traceback, zipfile
    ...
    if isinstance(value, dict):
        ...
        for role, factor in value.items():
            ...
            if not math.isfinite(number) or number <= 0:
                raise ValueError(f"reflectance_scale for '{role}' must be finite and > 0")
            clean[role] = number
...
def validate_config(config):
    ...
    ortho = _existing(config.get("orthomosaic_path") or config.get("multispectral_path"))
    if not ortho:
        raise ValueError("A GeoTIFF orthomosaic is required")
    supplied_band_map = config.get("band_map")
    clean = None
    if supplied_band_map:
        clean = {key: int(value) for key, value in supplied_band_map.items()
    ...
    offline_basemap = _as_bool(config.get("offline_basemap", True), name="offline_basemap")
    ...
    with rasterio.open(ortho) as dataset:
        if dataset.driver != "GTiff":
            raise ValueError(f"Input must be a GeoTIFF; detected driver {dataset.driver!r}")
        if dataset.crs is None:
            raise ValueError("Input GeoTIFF has no coordinate reference system")
        if clean is not None:
            too_high = {role: index for role, index in clean.items() if index > dataset.count}
            if too_high:
                details = ", ".join(f"{role}={index}" for role, index in too_high.items())
                raise ValueError(
        ...
        clean = detected_map
        raster_info = {
            "path": str(ortho), "bands": int(dataset.count),
...
def make_preview(ortho, out_path, band_map):
    ...
    with rasterio.open(ortho) as src:
        ...
        arr = src.read(ids, out_shape=(len(ids), min(src.height, 1600), min(src.width, 1600))).astype("float32")
    if arr.shape[0] == 1:
...
def run(config, progress_callback=None):
    ...
    result = run_agriculture_pipeline(
        ...
        reflectance_scale=scale, k_range=(zones, zones),
        ...
        allow_unvalidated_prescription_export=bool(config.get("unsafe_override", False)),
    ...
    with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED, allowZip64=True) as output_zip:
        for path in sorted(out.rglob("*")):
            if path.is_file():
                output_zip.write(path, path.relative_to(out.parent))
    if progress_callback:
        progress_callback("Complete. Deliverables ready", 100)
    return {"output_dir": str(out), "archive": str(archive), "core_outputs": result,
            "input_raster": raster_info}
...
def main(argv=None):
    ...
    try:
        ...
        print(json.dumps({"error": str(exc), "traceback": traceback.format_exc()}), file=sys.stderr, flush=True)
        return 1
